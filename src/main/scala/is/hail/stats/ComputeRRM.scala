...
object ToIndexedRowMatrix {
  def apply(vds: VariantDataset, useDosage: Boolean, sampleMask: Array[Boolean], completeSampleIndex: Array[Int]): IndexedRowMatrix = {
    require(vds.wasSplit)
    val n = vds.nSamples
    val indexedRows = 
      if (!useDosage)
    ...
    new IndexedRowMatrix(indexedRows, vds.countVariants(), n)
  }
}
